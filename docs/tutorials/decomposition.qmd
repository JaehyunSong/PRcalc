...
temp_obj1 <- obj1
temp_obj1$raw[14, ] <- c("Total", colSums(obj1$raw[, -1]))
temp_obj1$dist[14, ] <- c("Total", colSums(obj1$dist[, -1]))
temp_raw  <- temp_obj1$raw |> 
  mutate(across(-1, \(x) as.numeric(x)))
temp_dist <- temp_obj1$dist |> 
  mutate(across(-1, \(x) as.numeric(x)))
...
temp_raw |> 
  gt() |> 
  data_color(columns = 13, rows = 1:13, palette = "royalblue")
...
temp_raw |> 
  gt() |> 
  data_color(columns = 2:12, rows = 14, palette = "royalblue")
...
temp_raw |> 
  gt() |> 
  data_color(columns = 2:12, rows = 1:13, palette = "royalblue")
